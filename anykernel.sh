...
if [ -z $NEW ]; then
  ...
  ui_print " "
  ui_print "  Select CPU frequency options  : "
  ui_print "  + Volume Up = OC 1.8 GHz + 2.2 GHz"
  ui_print "  - Volume Down = Default frequency"
  if $FUNCTION; then
    NEW=true
  else
    NEW=false
...
compressed_image=$decompressed_image.gz
  ...
  if $NEW; then
    cat $compressed_image /tmp/anykernel/dtbs/OC/*.dtb > /tmp/anykernel/Image.gz-dtb;
  else
    cat $compressed_image /tmp/anykernel/dtbs/non-OC/*.dtb > /tmp/anykernel/Image.gz-dtb;
  fi
